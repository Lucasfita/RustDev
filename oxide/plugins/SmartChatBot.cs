...
{
    [Info("Smart Chat Bot", "i_love_code", 1.7, ResourceId = 2435)]
    ...
    public class SmartChatBot : RustPlugin
    {
        ...
        {
            string playerIdString = ((IPlayer)data["Player"]).Id;
            ...
            LogDebugMessage("BetterChat Override: Automated message found for chat message [" + message + "]");

            if (messageConfig.SendToServer || player.IsAdmin)
                return data;

            LogDebugMessage("BetterChat Override: Displaying [" + messageConfig.Message + "] directly to user");

            var allPlayerIds = BasePlayer.activePlayerList.Select(x => x.UserIDString).ToList();

            data["BlockedReceivers"] = allPlayerIds;

            return data;
        }

        private void LogDebugMessage(string message, params object[] args)
        {
            if(PluginConfiguration?.IsDebug ?? false)
            {
        ...
        object OnPlayerChat(ConsoleSystem.Arg arg)
        {
            ...
            LogDebugMessage("Trying to find autoresponse for [" + message + "]");

            var messageConfig = FindAutoresponseForMessage(message);

            if (messageConfig == null)
            {
                LogDebugMessage("No autoresponse found for message [" + message + "]");
                return null;
            }

            LogDebugMessage("Automated message found for chat message [" + message + "]. Will display [" + messageConfig.Message + "]");
            ...
            if (messageConfig.SendToServer)
            {
                ...
                {
                    LogDebugMessage("Preventing global automated message due to timed cooldown");
                    return true;
                }

                NextTick(() =>
                {
                    LogDebugMessage("Displaying [" + messageConfig.Message + "] to server");
            ...
            LogDebugMessage("Displaying [" + messageConfig.Message + "] directly to user");
            DisplayMessageToUser(player, $"{messageConfig.Message}");
