...
{
	[Info("BetterLoot", "FuJiCuRa / dcode", "2.15.0", ResourceId = 828)]
	...
	public class BetterLoot : RustPlugin
	{
		...
		StoredBradleyCrate storedBradleyCrate = new StoredBradleyCrate();
		...
		Dictionary<string,object> scrapAmounts = new Dictionary<string,object>();
		
		Regex barrelEx;
		Regex crateEx;
		Regex heliEx = new Regex(@"heli_crate");
		Regex bradleyEx = new Regex(@"bradley_crate");
		...
		List<string>[] itemsBradley = new List<string>[5];
		List<string>[] itemsSupply = new List<string>[5];
		int totalItems;
		int totalItemsB;
		int totalItemsC;
		int totalItemsHeli;
		int totalItemsBradley;
		...
		List<ItemDefinition> originalItemsBradley;
		...
		int minItemsPerBradleyCrate;
		int maxItemsPerBradleyCrate;
		...
		void LoadVariables()
		{
			...
			minItemsPerBradleyCrate = Convert.ToInt32(GetConfig("BradleyCrate", "minItemsPerBradleyCrate", 2));
			maxItemsPerBradleyCrate = Convert.ToInt32(GetConfig("BradleyCrate", "maxItemsPerBradleyCrate", 4));
			includeBradleyCrate = Convert.ToBoolean(GetConfig("BradleyCrate", "includeBradleyCrate", false));
			useCustomTableBradley = Convert.ToBoolean(GetConfig("BradleyCrate", "useCustomTableBradley", true));
			randomAmountBradleyCrate = Convert.ToBoolean(GetConfig("BradleyCrate", "randomAmountBradleyCrate", true));
		...
		void UpdateInternals(bool doLog)
		{
			...
			originalItemsBradley = new List<ItemDefinition>();
			...
			if (useCustomTableBradley && includeBradleyCrate)
				foreach (KeyValuePair<string, int> pair in storedBradleyCrate.ItemList)
					originalItemsBradley.Add(ItemManager.FindItemDefinition(pair.Key));
			...
			if (doLog)
			{
				...
				if (useCustomTableBradley && includeBradleyCrate)
					Puts("There are " + originalItemsBradley.Count + " items in the BradleyTable.");
				if (useCustomTableSupply && includeSupplyDrop)
			...
			for (var i = 0; i < 5; ++i)
			{
				...
				if (useCustomTableBradley && includeBradleyCrate) itemsBradley[i] = new List<string>();
			...
			if (useCustomTableBradley && includeBradleyCrate) totalItemsBradley = 0;
			...
			if (useCustomTableBradley && includeBradleyCrate)
			{
				foreach (var item in originalItemsBradley)
				{
					if (item == null) continue;
					int index = RarityIndex(item.rarity);
					object indexoverride;
					...
					if (ItemExists(item.shortname)) {
						if (!storedBlacklist.ItemList.Contains(item.shortname)) {
							itemsBradley[index].Add(item.shortname);
							++totalItemsBradley;
						}
					}
					else
					{
						++notExistingItemsBradley;
					}
				}
			}
			if (useCustomTableSupply && includeSupplyDrop)
			...
			for (var i = 0; i < 5; ++i) {
				...
				if (useCustomTableBradley && includeBradleyCrate) { totalItemWeightBradley += (itemWeightsBradley[i] = ItemWeight(baseItemRarity, i) * itemsBradley[i].Count); }
		...
		Item MightyRNG(string type) {
			...
			case "bradley":
								do {
									selectFrom = null;
									item = null;
									var r = rng.Next(totalItemWeightBradley);
									for (var i=0; i<5; ++i) {
										limit += itemWeightsBradley[i];
										if (r < limit) {
											selectFrom = itemsBradley[i];
											break;
										}
									}
									if (selectFrom == null) {
								...
								if (item == null)
									return null;
								if (item.info.stackable > 1 && storedBradleyCrate.ItemList.TryGetValue(item.info.shortname, out limit))
								{
									if (limit > 0)
									{
										if (randomAmountBradleyCrate)
											item.amount = rng.Next(1, Math.Min(limit, item.info.stackable));
										else
											item.amount = Math.Min(limit, item.info.stackable);
									}
		...
		void PopulateContainer(LootContainer container)
		{
			...
			else if (bradleyEx.IsMatch(container.gameObject.name) && includeBradleyCrate) {
				SuppressRefresh(container);
				ClearContainer(container);
				min = minItemsPerBradleyCrate;
				max = maxItemsPerBradleyCrate;
				if(useCustomTableBradley)
				{
					type = "bradley";
				}
				else
		...
		void GenerateScrap(LootContainer container)
		{
			ItemDefinition scrapDef = ItemManager.FindItemDefinition("scrap");
			if (scrapDef == null)
				return;
			int num = 0;
			if (scrapAmounts.ContainsKey(container.ShortPrefabName))
				num = (int)scrapAmounts[container.ShortPrefabName];
			if (num > 0)
			{
				container.inventory.capacity += 1;
				container.inventorySlots += 1;
				Item item = ItemManager.Create(scrapDef, num, 0uL);
				if (!item.CanMoveTo(container.inventory, -1, true))
				{
					container.SendNetworkUpdate();
					if (!item.MoveToContainer(container.inventory, -1, true))
						item.Drop(container.transform.position, Vector3.zero, default(Quaternion));
				}
				else
        ...
        {
			if ((Config.Get("ScrapAmounts") != null))
			{
				scrapAmounts = Config.Get("ScrapAmounts") as Dictionary<string, object>;
			}
			var filter = Oxide.Game.Rust.RustExtension.Filter.ToList();
			filter.Add("[BUNDLE] Not found in bundle");
			Oxide.Game.Rust.RustExtension.Filter = filter.ToArray();
			var allTypes = FileSystem.LoadAll<GameObject>("Assets/", ".prefab").Where(lc => lc.GetComponent<LootContainer>() != null).Cast<GameObject>().Select(lc => lc.GetComponent<LootContainer>().PrefabName).ToList();
			filter = Oxide.Game.Rust.RustExtension.Filter.ToList();
			filter.Remove("[BUNDLE] Not found in bundle");
			Oxide.Game.Rust.RustExtension.Filter = filter.ToArray();
			bool wasAdded = false;
			foreach (var allType in allTypes)
            {
				var loot = GameManager.server.FindPrefab(allType).GetComponent<LootContainer>();
				if (loot.ShortPrefabName.StartsWith("dm") || loot.ShortPrefabName.StartsWith("visual"))
					continue;
				if (!scrapAmounts.ContainsKey(loot.ShortPrefabName))
				{
					scrapAmounts.Add(loot.ShortPrefabName, loot.scrapAmount);
					wasAdded = true;
				}
			}
			if (wasAdded)
			{
				Config["ScrapAmounts"] = scrapAmounts;
				Config.Save();
			}
        }
		...
		class StoredBradleyCrate
		{
			public Dictionary<string, int> ItemList = new Dictionary<string, int>();

			public StoredBradleyCrate()
			{
			}
		}

		void LoadBradleyCrate()
		{
			storedBradleyCrate = Interface.GetMod().DataFileSystem.ReadObject<StoredBradleyCrate>("BetterLoot\\BradleyCrate");
			if (pluginEnabled && storedBradleyCrate.ItemList.Count > 0 && !includeBradleyCrate && !useCustomTableBradley)
				Puts("BradleyCrate > loot population is disabled by 'includeBradleyCrate'");
			if (pluginEnabled && storedBradleyCrate.ItemList.Count > 0 && !includeBradleyCrate && useCustomTableBradley)
				Puts("BradleyCrate > 'useCustomTableBradley' enabled, but loot population inactive by 'includeBradleyCrate'");
			if (storedBradleyCrate.ItemList.Count == 0)
			{
				includeBradleyCrate = false;
				Config["BradleyCrate","includeBradleyCrate"]= includeBradleyCrate;
				Changed = true;
				Puts("BradleyCrate > table not found, option disabled by 'includeBradleyCrate' > Creating a new file.");
				storedBradleyCrate = new StoredBradleyCrate();
				foreach(var it in ItemManager.itemList)
				{
					int stack = 0;
					if(!ItemExists(it.shortname)) continue;
					...
					if (it.category == ItemCategory.Component) continue;
					if (stack == 0) stack = 1;
					storedBradleyCrate.ItemList.Add(it.shortname,stack);
				}
				Interface.GetMod().DataFileSystem.WriteObject("BetterLoot\\BradleyCrate", storedBradleyCrate);
			}
		}

		void SaveBradleyCrate() => Interface.GetMod().DataFileSystem.WriteObject("BetterLoot\\BradleyCrate", storedBradleyCrate);

		#endregion BradleyCrate
