                ...
                try
                {
                    ...
                    foreach (var target in BasePlayer.activePlayerList.Where(target => target != null && target.transform != null && target.IsConnected))
                    {
                        ...
                        if (currDistance < playerDistance)
                        {
                            ...
                            if (drawText) player.SendConsoleCommand("ddraw.text", invokeTime + flickerDelay, color, target.transform.position + new Vector3(0f, 2f, 0f), string.Format("{0} <color={1}>{2}</color> <color={3}>{4}</color> {5}", target.displayName ?? target.userID.ToString(), healthCC, Math.Floor(target.health), distCC, currDistance, extText));
                    ...
                    if (showSleepers || showAll)
                    {
                        foreach (var sleeper in BasePlayer.sleepingPlayerList)
                        {
                            ...
                            if (currDistance < playerDistance)
                            {
                                ...
                                if (drawText) player.SendConsoleCommand("ddraw.text", invokeTime + flickerDelay, color, sleeper.transform.position, string.Format("{0} <color={1}>{2}</color> <color={3}>{4}</color>", sleeper.displayName, healthCC, Math.Floor(sleeper.health), distCC, currDistance));
